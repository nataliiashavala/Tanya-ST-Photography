...
<template>
  ...
  <div
    ...
    class="header flex items-end uppercase pt-0 lg:pt-9 pl-4 lg:pl-24 lg:pr-64 pb-14 lg:pb-0 font-lato tracking-widest">
  

    <!-- Sidebar -->
  <div class="absolute inset-y-0 left-0 bg-[#F5F5F5] text-black w-80 min-h-screen overflow-y-auto z-50 transition-transform transform -translate-x-full ease-in-out duration-300"
      id="sidebar">
  ...
  <div class="flex items-end justify-between lg:justify-start">
    <div class="pt-5 md:pt-0 ml-2 mr-0 lg:ml-24 lg:mr-24 ">
    <nuxt-link to="/home_temp" class="">
      <img :src="Logo" alt="Logo" :style="logoStyle" class="w-16 h-16 lg:w-full lg:h-full" />
    </nuxt-link></div>
    <div class="">
    <nuxt-link class="ml-40 mr-0 text-2xl uppercase font-nanum block lg:hidden cursor-pointer" id="open-sidebar">
      menu
    </nuxt-link>
  </div>
      ...
      <div class="relative custom-select-wrapper mr-20" v-if="isMenuOpen || windowSize.width >= 900">
        ...
        >
          ...
          <option 
            ...
            :value="option.path" 
            :style="optionStyle"
            class="font-lato text-xs uppercase tracking-widest "
          >
            {{ option.name }}
          </option>
        </select>
      </div>
      <nuxt-link to="/contact-me" class="mb-4 uppercase font-lato text-xs" v-if="isMenuOpen || windowSize.width >= 900">contact me</nuxt-link>
    </nav>
  </div>
  </div>
</template>
